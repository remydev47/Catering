'use client';
import React from 'react'

const Services = () => {
  return (
    <section className="pt-12 bg-gradient-to-b from-gray-50 via-white to-gray-50">
    <div className="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
        <div className="grid max-w-md grid-cols-1 mx-auto lg:grid-cols-12 gap-x-6 gap-y-8 lg:max-w-none">
            <div className="self-center lg:col-span-4">
                <h1 className="text-base font-bold text-gray-900 sm:text-4xl xl:text-5xl">Our Services</h1>
                <p className="mt-5 text-4xl font-serif leading-7  text-gray-500 ">PLANNING A <br/> FABULOUS EVENT ?</p>
                <div className="relative inline-flex mt-9 group">
                    <div className="absolute transitiona-all duration-1000 opacity-70 inset-0 bg-gradient-to-r from-[#44BCFF] via-[#FF44EC] to-[#FF675E] rounded-xl blur-lg filter group-hover:opacity-100 group-hover:duration-200"></div>

                    <a
                        href="/Menu"
                        title="services"
                        className="relative inline-flex items-center justify-center px-8 py-3 sm:text-sm sm:py-3.5 text-base font-semibold text-white transition-all duration-200 bg-orange-400 border border-transparent rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600"
                        role="button"
                    >
                       VIEW ALL SERVICES
                    </a>
                </div>
            </div>

            <div className="self-end lg:order-last lg:pb-20 lg:col-span-3">
                <p className="text-xs font-bold tracking-widest text-gray-500 uppercase">⚡️ Reasons to Pickus</p>

                <div className="mt-6 space-y-6 lg:space-y-8">
                    <div className="relative overflow-hidden">
                        <div className="flex items-start lg:items-center">
                             <h2 className='font-serif text-2xl text-gray-400'>1</h2>
                            <p className="ml-5 text-base font-bold font-serif leading-6 text-gray-900">
                                <a href="#" title="">
                                 WEDDING                                  
                                </a>
                                <br/>
                                <span className="text-gray-400 font-normal" >
                                 On one of the most important days of your life, we provide a full catering program
                                 </span>
                            </p>
                        </div>
                    </div>

                    <div className="relative overflow-hidden">
                        <div className="flex items-start lg:items-center">
                        <h2 className='font-serif text-2xl text-gray-400'>2</h2>
                            <p className="ml-5 text-base font-bold leading-6 text-gray-900">
                                <a href="#" title="">
                                  COOPORATE
                                </a>
                                <br/>
                                <span className="text-gray-400 font-normal">
                                 You can order a corporate full time or part-time service for personnel/staff lunches.
                                </span>
                            </p>
                        </div>
                    </div>

                    <div className="relative overflow-hidden">
                        <div className="flex items-start lg:items-center">
                        <h2 className='font-serif text-2xl text-gray-400'>3</h2>
                            <p className="ml-5 text-base font-bold leading-6 text-gray-900">
                                <a href="#" title="">
                                  SOCIAL EVENTS
                                </a>
                                <br/>
                                <span className="text-gray-400 font-normal">
                                 Social events are highly important to our clients, and we guarantee perfect service.
                                </span>
                            </p>
                        </div>
                    </div>

                    <div className="relative overflow-hidden">
                        <div className="flex items-start lg:items-center">
                        <h2 className='font-serif text-2xl text-gray-400'>4</h2>
                            <p className="ml-5 text-base font-bold leading-6 text-gray-900">
                                <a href="#" title="">
                                  PICNICS   
                                </a>
                                <br/>
                                <span className="text-gray-400 font-normal">
                                We will be more than happy to work for you during a family or a business picnic
                                </span>
                            </p>
                        </div>
                    </div>

                    <div className="relative overflow-hidden">
                        <div className="flex items-start lg:items-center">
                        <h2 className='font-serif text-2xl text-gray-400'>5</h2>
                            <p className="ml-5 text-base font-bold leading-6 text-gray-900">
                                <a href="#" title="">
                                  FESTIVALS
                                </a>
                                <br/>
                                <span className="text-gray-400 font-normal">
                                 We are completely capable of serving for big, open, small, adults or kids festivals.
                                </span>
                            </p>
                        </div>
                    </div>

                    <div className="relative overflow-hidden">
                        <div className="flex items-start lg:items-center">
                        <h2 className='font-serif text-2xl text-gray-400'>6</h2>
                            <p className="ml-5 text-base font-bold leading-6 text-gray-900">
                                <a href="#" title="">
                                  PRIVATE PARTIES
                                </a>
                                <br/>
                                <span className="text-gray-400 font-normal">
                                 A private dining room, or a penthouse catering is one of our many great services.
                                </span>
                            </p>
                        </div>
                    </div>


                </div>
            </div>

            <div className="self-end lg:col-span-5">
                <img className="w-full mx-auto rounded-full border-gray-300" src="/images/services.png" alt="" />
            </div>
        </div>
    </div>
</section>
  )
}

export default Services